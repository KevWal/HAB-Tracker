...
//	private GraphViewSeries viewseries;
    ...
    public void onCreate(Bundle savedInstanceState) {
      ...
      //  list = (ListView)findViewById(R.id.listRxDecodedStrings);
       ...
       // txtchars.setEnabled(false);
        ...
        //pass grad.png to Waterfall to set colour spectrum
    ...
    @Override
   	public void onResume() {
   		...
   		//fftreceiver
   		if (fftupdateReceiver == null) fftupdateReceiver = new FFTUpdateReceiver(this);
    ...
    */
    ...
    private class FFTUpdateReceiver extends BroadcastReceiver {
        ...
        //KEW Getting the application context to enable us to access shared preferences
        Context _context;
        public FFTUpdateReceiver(Context appContext){
            _context = appContext;
        }
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(Dsp_service.FFT_UPDATED)) {
                ...
                //KEW Before we draw the next line of the Waterfall, get the SignalVolume multiplier
                double signalVolume = Double.parseDouble(PreferenceManager.getDefaultSharedPreferences(_context).getString("pref_waterfall_sensitivity","1"));
                wf.setSignalVolume(signalVolume);

                Bitmap img = wf.updateLine(mService.getFFT(),mService.get_f1_FFTbin(),mService.get_f2_FFTbin());
				//img = wf.updateLine(mService.getFFT(),mService.get_f1_FFTbin(),mService.get_f2_FFTbin());
            	if (img != null){
    ...
    {
    	//Here we can set any value for baud rate like in if else conditions
    ...
    {
    	//Here we can enable or disable Echo voice
